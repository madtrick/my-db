...
describe('Dedup transactions', () => {
  it('removes duplicates from different csv collections', () => {
    ...
    // Remove the hash from the transaction object so that the expectation
    // passs. In the future we could use somethine like
    ...
    expect(dedupedTransactionsNoHash).to.eql([
      { Category: 'Foo', Amount: -14.00, Reference: 'xyz' },
      { Category: 'Foo', Amount: -14.00, Reference: '123z' },
      { Category: 'Foo', Amount: -15.00, Reference: 'ert' },
      { Category: 'Bar', Amount: -13.00, Reference: '' },
    ]);
  });
});
